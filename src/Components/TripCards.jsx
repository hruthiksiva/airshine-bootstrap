...
const TripCards = ({ trips, slice }) => {
  ...
  if (slice) {
    filteredTrips = filteredTrips.slice(0, 3);
  }
        ...
        <div key={trip.id} className="col">
          <div className="trip-card card h-100">
            <div className="position-relative overflow-hidden">
              <img src={trip.image} alt={trip.location} className="trip-image w-100" />
            </div>
            <div className="card-body d-flex flex-column">
              <h3 className="trip-location">{trip.location}</h3>
              <p className="trip-duration mb-4">{trip.duration}</p>
              <div className="mt-auto">
                <a href={trip.link} className="explore-btn d-inline-block">More</a>
              </div>
            </div>
          </div>
        </div>
